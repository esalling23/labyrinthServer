...
//           }).catch((err) => { console.log(err) });
  ...
  controller.on("map_event", function(options) {
    ...
    controller.on("before_hook", function(bot, teamId, script) {
      
      // This door leads to the puzzle thread as set up in BotKit Studio
      // The bot "replies" with what the user said
      
      var name = script.name ? script.name : script;
      // Based on the format of "message", set the teamId
            console.log(name, "before hook script");
     ...
     // var galaxy = script.name.split("_")[0] + "_" + script.name.split("_")[1];
      ...
      // console.log(message);
        ...
        controller.studio.before(name, function(convo, next) {
          // console.log(convo.messages[0].attachments[0].actions);
          
          
          // Find team to check puzzles data                   
          controller.storage.teams.get(teamId, function(err, team) {
                                      
            // console.log("the team are: ", team.id);
