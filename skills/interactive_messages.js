              ...
              if (!puzzle.locked || !locked) {
                ...
                bot.reply(message, "This door is unlocked. Sending you to the room.", (err, response) => {
                  ...
                  setTimeout(function() {
                    ...
                    // console.log(puzzleName, "right before the before hook");
                    // Use the script name to do some stuff before it runs
                      controller.trigger("before_hook", [bot, teamId, puzzleName]);

                      // Delete the original message the bot sent to the player          
                      bot.api.chat.delete({ts: response.ts, channel: response.channel}, function(err, deletedMessage) {
                        // console.log("deleted: ", message);
                        // controller.studio.run(theBot, puzzleName, message.user, message.channel)
                        //     .catch((err) => {
                ...
                console.log(puzzleName, "is locked so here we go");
                
                
                // Use the script name to do some stuff before it runs
                controller.trigger("before_hook", [bot, teamId, scriptObj]);
        ...
        if (message.actions[0].name.match(/^confirm$/)) {
            ...
            controller.studio.getScripts().then((list) => {
              ...
              if (confirmedChoice.valid) {
                ...
                // Use the script name to do some stuff before it runs
               controller.trigger("before_hook", [bot, teamId, script]);
